      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {onOpenApiKeyModal && (
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md border transition-all font-mono text-xs font-bold cursor-pointer ${
                isDarkMode 
                  ? 'border-rose-500/40 bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 animate-pulse' 
                  : 'border-rose-300 bg-rose-50 hover:bg-rose-100 text-rose-700 shadow-xs'
              }`}
              title="Thiết lập Model AI & Nhập Gemini API Key cá nhân"
            >
              <KeyRound className="w-3.5 h-3.5 text-rose-500" />
              <span>Settings (API Key)</span>
              <span className={`text-[10px] hidden xl:inline ml-1 font-extrabold ${
          ...
          <div className={`flex items-center gap-2 px-3 py-1.5 rounded-md border text-xs font-mono ${
            ...
            <span className={`text-[11px] uppercase tracking-wider hidden md:inline ${isDarkMode ? 'text-zinc-400' : 'text-slate-500'}`}>Phòng:</span>
            <select
              ...
              className={`bg-transparent border-none font-bold focus:ring-0 cursor-pointer text-xs ${
                isDarkMode ? 'text-emerald-400' : 'text-teal-700'
              }`}
            >
              {classList.map(cls => (
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md border text-xs font-mono font-bold transition-all cursor-pointer ${
                isDbConnected
                  ...
                  : (isDarkMode ? 'bg-rose-500/10 border-rose-500/30 text-rose-400 hover:bg-rose-500/20' : 'bg-rose-50 border-rose-200 text-rose-700 hover:bg-rose-100')
              }`}
        ...
        <div className="flex items-center gap-3">
          ...
          {onOpenProfileEdit && (
            ...
            >
              <div className="relative w-7 h-7 rounded-full overflow-hidden border border-teal-500 shrink-0">
              ...
              <div className="text-left hidden md:block font-mono">
                <span className={`text-xs font-bold block line-clamp-1 leading-tight ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>{activeName}</span>
                <span className={`text-[9px] block leading-tight ${isDarkMode ? 'text-emerald-400' : 'text-teal-600 font-semibold'}`}>Chỉnh sửa hồ sơ ✎</span>
              </div>
            </button>
          )}
          ...
          {onOpenStudentLogin && (
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md border text-xs font-mono transition-all cursor-pointer ${
                role === 'STUDENT'
                  ? (isDarkMode ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-teal-50 border-teal-300 text-teal-700 font-bold')
          ...
          {role === 'STUDENT' ? (
            ...
            <div className={`hidden lg:flex items-center gap-2 text-xs font-mono ${isDarkMode ? 'text-zinc-400' : 'text-slate-500'}`}>
              <Sparkles className="w-3.5 h-3.5 text-teal-600" />
              <span className="text-[11px]">LMS Engine: <span className="text-teal-600 font-bold">Active</span></span>
            </div>
          )}
